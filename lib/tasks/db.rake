namespace :db do
	...
	namespace :data do
      ...
      YamlDb.load file
		...
		desc "Load contents of URL into database (for amazon s3 on heroku)  Use url= to specify the yaml file"
		task(:load_url => :environment) do
		  require 'net/http'
		  url = URI.parse(ENV['url'])
      file_name = File.join(RAILS_ROOT, 'tmp', Time.now.to_i.to_s)
