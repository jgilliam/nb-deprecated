class PrioritiesController < ApplicationController
  ...
  def index
    if params[:q] and request.xhr?
      @priorities = Priority.find(:all, :select => "priorities.name", :conditions => ["status = 'published' and name LIKE ? and endorsements_count > 0", "%#{h(params[:q])}%"], :order => "endorsements_count desc")
